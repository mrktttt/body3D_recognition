...
class SkeletonAnimatorVedo:
    ...
    # Option 1 du menu: Afficher une frame donnée.
    def create_skeleton_for_frame(self, frame_nb):
        ...
        # Créer les sphères pour les articulations
        joint_objects = []
        for joint_name, position in joint_positions.items():
            sphere = Sphere(position, r=0.05, c='red')
            sphere.name = joint_name
            joint_objects.append(sphere)
        
        # Créer les lignes pour les os
        bone_objects = []
        for start_bone, end_bone in self.bone_connections:
            if start_bone in joint_positions and end_bone in joint_positions:
                ...
                line = Line(start_pos, end_pos, c='blue', lw=5)
    ...
    def update_skeleton(self):
        ...
        # Ajouter un texte pour afficher le numéro de frame
        frame_text = Text3D(f"Frame: {self.current_frame + 1}/{len(self.animation_data)}", 
                           pos=(-2, -2, 2), s=0.1, c='white')
        self.plotter.add(frame_text)
        
        # Instructions
        instructions = Text3D("ESPACE/→: Frame suiv. | ←: Frame prec. | R: Début | P: Play/Pause | Q: Quitter", 
                             pos=(-2, -2, 1.5), s=0.05, c='yellow')
    ...
    def animate_interactive(self):
        ...
        self.plotter = Plotter(title="Animation Squelette 3D - Interactive", axes=1)
    ...
    def animate_continuous(self, fps=20):
        #Animation continue automatique
        self.plotter = Plotter(title="Animation Squelette 3D - Continue", axes=1)
        self.plotter.background('black')
    ...
    def animate_with_timer(self, fps=20):
        #Animation avec timer Vedo
        self.plotter = Plotter(title="Animation Squelette 3D - Timer", axes=1)
        self.plotter.background('black')
        ...
        self.plotter.timer_callback('create', dt=int(1000/fps))  # Interval en ms
    ...
    def show_single_frame(self, frame_idx=0):
        ...
        self.plotter = Plotter(title="Animation Squelette 3D - Frame statique", axes=1)
        self.plotter.background('black')
